-- Script to drop existing functions before recreating them
-- This fixes the "cannot change return type of existing function" error

-- Drop existing functions first
DROP FUNCTION IF EXISTS public.reset_profile_policies();
DROP FUNCTION IF EXISTS public.fix_missing_profiles();
DROP FUNCTION IF EXISTS public.test_storage_permissions();
DROP FUNCTION IF EXISTS public.test_profile_permissions(text);
DROP FUNCTION IF EXISTS public.ensure_user_is_admin(text);
DROP FUNCTION IF EXISTS public.make_user_admin(text);

-- Now import missing_functions.sql
-- Copy the contents from missing_functions.sql
